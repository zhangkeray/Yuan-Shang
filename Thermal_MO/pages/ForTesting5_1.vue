<template>
  <v-app>
    <v-main>
      <v-row>
        <v-col cols="8">
          <div ref="lineBarChart" style="height: 400px; width: 1250px"></div>
        </v-col>
        <v-col cols="2">
          <v-row>
            <v-col cols="6">
            ...
            <v-col cols="6">
              <v-btn @click="go()" color="primary" class="mt-3">確定</v-btn>
            </v-col>
          </v-row>
        </v-col>
      </v-row>
  ...
  methods: {
    ...
    getData() {
      axios
        ...
        .then((response) => {
          console.log(response.data.arr[5])
    ...
    go() {
      // document.getElementById("gg").refreshTimeNormal
    },
    drawBar() {
      ...
      // 此处是将时间戳改为正常Date格式时间
      var now = new Date(
        initTime.getTime() - queueLength * this.refreshTimeNormal
      ) // 当前时间减去前几分钟，以填充数据
      ...
      option = {
        ...
        series: [
          // 曲线设置
          {
            ...
            //   normal: {
            //     color: function () {
            //       if (opinionData.arr > 42 && opinionData.arr < 44) {
            //         color: 'grey'
            //       }
            //     },
            //   },
